"""Native cognis-connect emit for dripcheck — forward findings to any platform.

Maps dripcheck's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    dripcheck ... --format json | dripcheck-emit --to stix
    dripcheck-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
