...
**dripcheck** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `dripcheck` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
dripcheck ... --format json | cognis-connect emit --to sigma   # Sigma rules
dripcheck ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
dripcheck ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
dripcheck ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `dripcheck` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="dripcheck") for rec in records]   # records = your JSON output
